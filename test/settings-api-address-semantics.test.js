...
test('保存设置时会保留已有 providerDefaults，避免移除服务端默认配置 UI 后清空旧配置', () => {
  ...
  assert.doesNotMatch(app, /\$\('#serverDefaultImageModel'\)|\$\('#serverDefaultResponsesModel'\)|\$\('#serverDefaultStreamMode'\)/);
});
