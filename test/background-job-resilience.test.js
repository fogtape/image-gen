...
test('前端后台任务轮询增加超时、有限重试，并把直连回退限制在建任务阶段', () => {
  ...
  const backgroundJobs = read('frontend/background-jobs.js');
  const http = read('frontend/http.js');
  assert.match(backgroundJobs, /const BACKGROUND_JOB_CREATE_TIMEOUT_MS = 20_000/);
  assert.match(backgroundJobs, /const BACKGROUND_JOB_POLL_RETRY_LIMIT = 4/);
  assert.match(http, /async function fetchWithTimeout\(url, options = \{\}, timeoutMs = 15_000\)/);
  assert.match(app, /async function pollBackgroundJob\(jobId, format, isOAuth(?:, resultMeta = \{\})?\) \{[\s\S]*retryCount = 0[\s\S]*retryCount \+= 1[\s\S]*BACKGROUND_JOB_POLL_RETRY_LIMIT[\s\S]*backgroundJobBackoffMs/s);
...
test('服务端图片上游请求带统一超时保护，超时按 504 处理', () => {
  ...
  assert.match(server, /async function runWithTimeout\(task, timeoutMs, timeoutMessage, externalSignal = null\)/);
  assert.match(server, /JOB_CANCELLED/);
